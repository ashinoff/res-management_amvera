...
- **2026-08-07** — Мобильная версия, ЭТАП 4 (Структура сети). До 1024px
  ...
  (`.ns-tp`) с именем и числом ВЛ; тап (`expandedTp`) раскрывает список ВЛ; у
  каждой ВЛ — 3 позиции (начало/середина/конец) крупными тапабельными ячейками
  `.ns-cell` (номер ПУ + цвет статуса через `getStatusColor`). Клик по ячейке —
  тот же `handleCellClick` (открытие ПУ/`ExtendedPuModal`), двойной — `startEdit`
  (при праве). Поиск по ТП (`.structure-controls`) — `sticky top`, во всю ширину.
  `ExtendedPuModal` — на `ModalShell`, полноэкранная уже из этапа 3. Десктопная
  сетка/секции/АСКУЭ не тронуты. `npm run build` — ОК.
...
- **2026-08-01** — «Загруженные документы»: переключатель вида **Таблица / Плитка**
  ...
  вложение через готовый `BlobImage` (грузит через прокси `/api/f`), для PDF/без
  картинок — иконка; бейдж кол-ва файлов, чекбокс массового выбора, кнопки просмотр/
  удаление — как в таблице (те же обработчики/права `files_manage`). Клик по
  миниатюре открывает `FileViewer`. Только `App.jsx` + новый CSS `.doc-view-*`/
  `.documents-grid`/`.doc-card*` (существующие стили не тронуты). `vite build` — ОК.
- **2026-08-01** — «Глубокий анализ работ», детализация «почему фикция». Бэкенд:
  новый сигнал **fast** — дней между `initialCheckDate`→`workCompletedDate`; параметр
  `fastDays` (дефолт 1); закрытие в ≤fastDays дн. считаем мгновенным (для выезда
  невыполнимо) → per-РЭС `fast:{count,share,days}`, reason и +15 к баллу при доле
  >0.5 и ≥5 закрытий. `suspectTp[]` теперь содержит `records[]` (`puNumber, checkDate,
  workDate, days`), `minDays`, `fast`, а строки `problems[]` дополнены пояснением
  «почему считаем фикцией». `templateComments[]` дополнены `tps[]` (группа ТП, до 50).
  Фронт (`App.jsx`): поле «Мгновенно, дней»; в таблице «ТП для ручной проверки» —
  колонка «Проверка → мероприятия (дней)» с красной подсветкой мгновенных закрытий
  (`.da-fast`) и колонка «Почему считаем фикцией»; шаблонные комментарии выведены
  списком с группой ТП (`.da-tmpl*`); экспорт: колонка «% мгновенных» в «Сводке» +
  листы «Сроки» (по записям, «Мгновенно» красным) и «Шаблоны» (текст + группа ТП).
  `node --check` — ОК, `vite build` — ОК.
- **2026-08-01** — «Глубокий анализ работ»: в ответ роута добавлен `suspectTp[]`
  по каждому РЭС — перечень подозрительных ТП для ручного разбора с пояснением
  «в чём проблема» (`{tpName, closedCount, problems[]}`). Проблемы собираются в том
  же цикле: пакетное закрытие (счётчик по `batchTpCount`), шаблонный комментарий
  (из flagged-групп `commentMap`), дубль фото (из flagged `photoMap`), закрытия в
  нерабочее время, провалы перепроверки; сортировка по числу проблем→закрытий→имени.
  Фронт (`App.jsx`): в раскрытии карточки таблица «ТП для ручной проверки» (ТП /
  закрытий / список проблем) после «Причины оценки»; в экспорт добавлен лист «ТП»
  (РЭС/ТП/Закрытий/Проблема, по строке на проблему). `node --check` — ОК, `vite build` — ОК.
...
- **2026-08-01** — Доработка дедупа структуры (#2, до деплоя). Оставляем строку МИН id
  ...
  как есть; `sectionId` — первый НЕ-NULL от новых к старым (`array_agg(... ORDER BY id
  DESC) FILTER (... IS NOT NULL))[1]`), чтобы не откатить номера ПУ и не потерять
  привязку ВЛ↔секция. Перевес входящих FK и уникальный индекс — как были. Добавлено
  логирование `[dedup NetworkStructures] ДО/ПОСЛЕ чистки: групп-дублей N, строк к
  удалению M` (postgres, count по GROUP BY … HAVING count>1) — видно в логах Amvera.
  Идемпотентно (HAVING count>1). node --check — ОК.
...
- **2026-07-31** — «Превышение Pном»: развели этапы по принципу «каждый видит свою
  ...
  + `awaiting_recheck`; res_responsible: `res_work` + `awaiting_recheck`; админ/супер:
  все; без права и не РЭС/админ: пусто; обе роли доступа — объединение. РЭС-скоуп
  (свой resId у не-админа) сохранён. Явный `?stage`: у админа — как есть, у не-админа
  — только из своей области (чужой этап → `[]`; напр. АСКУЭ `?stage=res_work` → пусто).
  Бейдж (`getNotificationCounts`): только РАБОЧИЕ кейсы — АСКУЭ `askue_limit` (свой
  РЭС), РЭС `res_work` (свой РЭС), админ `askue_limit`+`res_work` (все);
  `awaiting_recheck` в бейдж НЕ входит ни у кого (было: у АСКУЭ/админа входил — убрано,
  админ раньше не считал res_work — добавлено). Frontend `PowerOverload`: вкладки по
  роли/праву вместо «Активные/Завершённые» — АСКУЭ: «К ограничению (N)» +
  «Ожидают перепроверки (M)»; РЭС: «Мероприятия РЭС (N)» + «Ожидают перепроверки»;
  админ: все этапы + «Завершённые» + фильтр РЭС; активная вкладка фильтрует `cases` по
  этапу, счётчики в лейблах. Read-only карточки awaiting_recheck: подсказка «Ожидает
  загрузки профиля мощности после отчёта РЭС от <дата>» (`.po-await-hint`). Кнопка
  «Мероприятия выполнены» теперь и у админа (бэкенд res-complete и так пускал admin).
  Адресация уведомлений power_overload по этапам (askue→право+админ, res→РЭС+админ)
  была сделана ранее (audience) — проверено, цела, не менялась. node --check /
  npm run build — ОК.
- **2026-07-30** — «Превышение Pном»: значки этапов перенесены ВПРАВО от текста. Блок
  `poStages` вынесен из `.notification-narrow-info` и стал соседом справа (info имеет
  `flex:1` → значки уезжают вправо, `.notification-narrow-content` `align-items:center`
  → по центру высоты карточки). CSS `.po-stages` — `flex-shrink:0`, без `margin-top`.
  npm run build — ОК.
- **2026-07-30** — «Превышение Pном»: значки этапов в теле карточки + чистка шапки.
  (1) На каждой карточке кейса — 3 крупных SVG-значка этапов: ввод ограничения АСКУЭ
  (`IconStageLimit` — манометр), мероприятия РЭС (`IconWrench`), перепроверка профилем
  (`IconStageRecheck` — столбцы+галочка). Свечение по состоянию: выполнено — зелёное
  (`ico-glow-green`), ожидание/текущий — оранжевое (`ico-glow-amber` добавлен), ещё не
  начато — приглушённое (`ico-dim`); формат как у значков АСКУЭ (статичное,
  hover-усиление, .12s). Состояние — `stageState(c, idx)` по `stageOrder`
  (askue_limit→res_work→awaiting_recheck→completed). Tooltip: «<этап> — выполнено/
  ожидание/ещё не начато». (2) Из шапки модалки убрано слово «перегруз»: `dockTitle`
  без «Перегруз:», пилюля статуса активного кейса теперь показывает этап
  (`STAGE_RU[c.stage]`) вместо «Перегруз». (3) Адресация уведомлений по этапам
  (res_work→РЭС, askue_limit+awaiting_recheck→АСКУЭ, админ→всё) уже была реализована
  ранее — проверено, цела, не менялась. npm run build — ОК.
- **2026-07-30** — Данные АСКУЭ: числа опущены на линию квадратов ПУ. Значки остаются
  в строке заголовка секции, а числа (потребители/мощности/сумма) теперь рендерятся в
  askue-ячейках ПЕРВОЙ строки ВЛ секции — на той же горизонтали, где цветные квадраты
  ПУ (`renderVlRow(item, false, askueNumberCells(section))` для i===0; остальные ВЛ —
  пустые ячейки). Отдельная строка `askueNumRow` осталась fallback'ом только для секций
  без привязанных ВЛ. Выравнивание: `.vl-row .askue-cell { align-self:start;
  padding-top:8px }` — числа на уровне квадратов. Сетка (11 колонок) не изменилась.
  npm run build — ОК.
- **2026-07-30** — Данные АСКУЭ, косметика колонок секции (по правкам владельца).
  Значки в заголовке крупнее +30% (18→24px). Свечение по смыслу: «Опрашиваемые» (P,
  бывш. «введённые ограничения») — зелёное (`.ico-glow-green` добавлен), «Не
  опрашиваемые» — красное (`.ico-glow-red`); «Потребители» и «Сумма» — синее.
  Числа в теле секции крупнее (`.askue-num` 16px, tabular-nums) с мелкими единицами
  (`.askue-unit` 9px): потребители «N шт», мощности «N кВт». Колонки расширены под
  число+единицу (54/66/66/66px), min-width 940, мобилка обновлена. Терминология
  «опрашиваемые/не опрашиваемые» — в tooltip'ах и подписях модалки. Значки остаются
  в заголовке, числа — в теле (`section-askue-row`). npm run build — ОК.
- **2026-07-30** — Данные АСКУЭ по перегрузу, коммит 2/2 (frontend). (1) Модалка
  завершения этапа АСКУЭ: сетка `.askue-grid` — 3 числовых поля (потребители,
  введённые ограничения кВт, не введённые/нет опроса кВт) + read-only «Общая
  разрешённая мощность ТП» (живая сумма limited+unpolled); валидация на вводе
  (мощности 0–9999, потребители 1–999) с текстом ошибки; отправляются в
  askue-complete. (2) «Структура сети», карточка секции: в общую `net-grid`
  вставлены 4 фикс-колонки ЛЕВЕЕ «Начало» (44/54/54/54px). В строке заголовка секции
  (`colHeader`) — 4 SVG-значка (потребители=`IconUsers`; введённые=`IconAskueLimited`
  «P»+wifi; не введённые=`IconAskueUnpolled` «P»+перечёркнутый wifi; сумма=`IconAskueSum`
  «ΣP») со свечением `.ico-glow-blue` (+hover-усиление .12s, как у ред./удал.). В
  отдельной строке тела секции (`section-askue-row`) — числа под своими значками
  (`tabular-nums`, по центру колонки, без «кВт»); без данных — приглушённый «—».
  Ширины резервируются всегда; VL-строки и заголовки получили пустые askue-ячейки —
  сетка (11 колонок) выровнена по всем карточкам. Tooltip: «<величина> · данные АСКУЭ
  от <дата>» / «данных АСКУЭ нет». Мобилка (≤640px): мельче значки/числа/колонки, без
  переносов (горизонтальный скролл `.net-grid min-width`). `IconUsers` добавлен в
  импорт. В «Карте опроса» не дублируется. npm run build — ОК.
...
- **2026-07-30** — Перегруз Pном, коммит 2/3: «болтается 2+ раза» → «Проблемные ТП».
  ...
  cycles, этап, пики (peakKw/recheckPeakKw), комментарии АСКУЭ/РЭС, вложения, даты
  этапов. В `processProfileFile` при инкременте цикла добавлено эскалация-уведомление
  ОДИН раз при переходе `cycles 1→2` (`newCycles === PROBLEM_TP_CYCLES`): power_overload
  «ТП … переведена в проблемные: мероприятия не дали результата (цикл 2)», в errorData
  `problemTp:true` (существующее «Повторный перегруз (цикл N)» осталось для workflow).
  Выход — кейс дошёл до `completed` (исчезает из списка). **Бейдж:** выбран учёт
  проблемных ТП в СУЩЕСТВУЮЩЕМ счётчике меню «Проблемные ВЛ» (`problem_vl` += count
  проблемных ТП, admin) — без отдельного бейджа, т.к. это один раздел с вкладками.
  Frontend `ProblemVL`: вкладки «Проблемные ВЛ»/«Проблемные ТП (перегруз)» (`.po-tabs`),
  компоненты `ProblemTpList` (карточки в стиле problem-card: шапка «ТП · СШ», res-бейдж,
  красный «N циклов», бейдж этапа `.po-stage-badge` цветом) и `ProblemTpModal` (история:
  пики, комментарии АСКУЭ/РЭС, фото через существующий FileViewer). node --check /
  npm run build — ОК.
