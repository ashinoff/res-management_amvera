...
Роли ПО прежние (`admin`/`uploader`/`res_responsible`); поверх роли `admin` есть
**суперадмин** и **гранулярные права**. Механизм расширяет ТОЛЬКО админов —
`uploader`/`res_responsible`/`uec_responsible` он не касается.
...
- **Каталог прав** — единственный источник **`PERMISSIONS`** в `backend/server.js`
  (ключ → русское название). Права хранятся в `User.permissions` (JSONB), в JWT НЕ
  вшиваются (смена без перелогина; истина — БД + кеш `accessCache`, TTL 60с).
- **Как закрыть роут правом:** после `checkRole([...])` добавить
  `requirePerm('<ключ>')`. Логика: суперадмин→пропуск, `admin`+`permissions[ключ]`→
  пропуск, `admin` без права→403 `{error,permission,title}`, НЕ-админ→насквозь
  (поэтому requirePerm безопасен и на смешанных роутах вроде poll-map/sync). Только
  суперадмин: `requireSuper`. `DELETE_PASSWORD` остаётся вторым фактором ПОВЕРХ права.
- **Как добавить НОВОЕ право:** (1) ключ+название в `PERMISSIONS`; (2)
  ...
  через `hasPerm(user,'ключ')` (модульный хелпер в `App.jsx`); при желании — пункт/
  вкладку. Управление правами: раздел «Права доступа» (только суперадмин, пункт меню
  `superOnly`), роуты `GET/PUT /api/admin/permissions[/:userId]`.
